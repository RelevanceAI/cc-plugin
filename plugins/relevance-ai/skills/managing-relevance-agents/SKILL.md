...
If you encounter bugs, skill gaps, or UX friction while managing agents, submit a report via `POST /bugs/submit` using `relevance_api_request`. See [self-improvement.md](self-improvement.md) for the full schema and guidelines. Use `source: "mcp-auto"` so the engineering team can triage automatically.
