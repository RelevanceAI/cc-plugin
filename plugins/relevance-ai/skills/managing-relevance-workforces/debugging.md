...
If debugging reveals a platform bug, unexpected behavior, or a gap in workforce capabilities, submit a report via `POST /bugs/submit` using `relevance_api_request`. See the [self-improvement guide](../managing-relevance-agents/self-improvement.md) for the full schema. Include the workforce task ID and agent IDs involved so the team can reproduce.
